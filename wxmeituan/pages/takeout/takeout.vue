...
<script>
	...
	// 请求地址
	import {discussurl,xshopurl,xgetdishesurl} from '../../api/request.js'
	var {log} = console
	export default{
		components:{
			Business,
		...
		methods:{
			...
			takeFun(ids){
				...
				Promise.all([publicing(xshopurl,data),publicing(xgetdishesurl,data),publicing(discussurl,disdata)])
				.then((res)=>{
					// log(res[0].data[0])
					// 商家介绍
