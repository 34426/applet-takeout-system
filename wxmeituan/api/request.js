// 在线接口
// let url = 'http://meituan.thexxdd.cn/api/'
// 测试接口
let url = 'http://localhost:3000/api/'

// 为你优选
let getpreferurl = `${url}forshop/xgetprefer`

// 附近商家
let wxshopurl = `${url}forshop/wxshop`

// 综合排序
let startingurl = `${url}forshop/starting`

// 筛选多选
let multipleurl = `${url}forshop/multiple`

//搜索
let searchurl = `${url}forshop/search`

// 评论
let discussurl = `${url}message/discuss`

// 商家介绍
let shopurl = `${url}forshop/shop`
// 商家介绍
let xshopurl = `${url}forshop/xshop`
// 商品数据
let getdishesurl = `${url}forshop/getdishes`
// 商品数据
let xgetdishesurl = `${url}forshop/xgetdishes`
// 登录
let wxloginurl = `${url}wxuser/wxlogin`

// 微信支付
let wxpayingurl = `${url}wxpay/wxpaying`

// 提交评论
let commenturl = `${url}message/comment`


export{getpreferurl,wxshopurl,startingurl,multipleurl,searchurl,discussurl,shopurl,getdishesurl,
wxloginurl,wxpayingurl,commenturl,xshopurl,xgetdishesurl}